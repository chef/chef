  ...
  class HTTP
    class AuthCredentials
      ...
      def signature_headers(request_params = {})
        fail ArgumentError, 'Cannot sign the request without a client name, check that :node_name is assigned' if client_name.nil?
        ...
        signed.reduce({}) { |memo, kv| memo["#{kv[0].to_s.upcase}"] = kv[1]; memo }
      end
    end
  end
end
