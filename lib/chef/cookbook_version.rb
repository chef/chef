...
class Chef
  ...
  class CookbookVersion
    ...
    def relative_paths_by_name(records)
      records.select { |record| record[:name] =~ /\.rb$/ }.to_h { |record| [File.basename(record[:name], ".rb"), record[:path]] }
    end

    # For each manifest record, produce a mapping of base filename (i.e. recipe name
    # or attribute file) to on disk location
    def filenames_by_name(records)
      records.select { |record| record[:name] =~ /\.rb$/ }.to_h { |record| [File.basename(record[:name], ".rb"), record[:full_path]] }
    end
