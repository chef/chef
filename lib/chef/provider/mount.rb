...
class Chef
  class Provider
    class Mount < Chef::Provider
      ...
      def mounted?
        raise Chef::Exceptions::UnsupportedAction, "#{to_s} does not implement #mounted?"
      end

      # should check new_resource against current_resource to see if mount options need updating, returns true/false
      def mount_options_unchanged?
        raise Chef::Exceptions::UnsupportedAction, "#{to_s} does not implement #mount_options_unchanged?"
      end
      ...
      def mount_fs
        raise Chef::Exceptions::UnsupportedAction, "#{to_s} does not support :mount"
      end

      # should implement unmounting of the filesystem, raises if action does not succeed
      def umount_fs
        raise Chef::Exceptions::UnsupportedAction, "#{to_s} does not support :umount"
      end
      ...
      def remount_fs
        raise Chef::Exceptions::UnsupportedAction, "#{to_s} does not support :remount"
      end

      # should implement enabling of the filesystem (e.g. in /etc/fstab), raises if action does not succeed
      def enable_fs
        raise Chef::Exceptions::UnsupportedAction, "#{to_s} does not support :enable"
      end

      # should implement disabling of the filesystem (e.g. in /etc/fstab), raises if action does not succeed
      def disable_fs
        raise Chef::Exceptions::UnsupportedAction, "#{to_s} does not support :disable"
      end
