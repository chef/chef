...
class Chef
  class Provider
    class Subversion < Chef::Provider
      ...
      alias_method :revision_slug, :revision_int
      ...
      def extract_revision_info(svn_info)
        repo_attrs = svn_info.lines.reduce({}) do |attrs, line|
        ...
        fail "Could not parse `svn info` data: #{svn_info}" if repo_attrs.empty?
      ...
      def assert_target_directory_valid!
        ...
        unless ::File.directory?(target_parent_directory)
          ...
          fail Chef::Exceptions::MissingParentDirectory, msg
        end
      end
    end
  end
