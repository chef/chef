...
class Chef
  class Cookbook
    class CookbookVersionLoader
      ...
      def load_root_files
        Chef::Util::PathHelper.glob(File.join(cookbook_path, '*'), File::FNM_DOTMATCH).each do |file|
      ...
      def load_recursively_as(category, category_dir, glob)
        ...
        Chef::Util::PathHelper.glob(file_spec, File::FNM_DOTMATCH).each do |file|
          next if File.directory?(file)
      ...
      def load_as(category, *path_glob)
        Chef::Util::PathHelper.glob(File.join(cookbook_path, *path_glob)).each do |file|
          cookbook_settings[category][file[@relative_path, 1]] = file
        end
      end
