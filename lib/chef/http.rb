  ...
  class HTTP
    ...
    class StreamHandler
      ...
      def handle_chunk(next_chunk)
        ...
        @stream_handlers.reverse.reduce(next_chunk) do |chunk, handler|
    ...
    def apply_request_middleware(method, url, headers, data)
      middlewares.reduce([method, url, headers, data]) do |req_data, middleware|
    ...
    def apply_response_middleware(response, rest_request, return_value)
      middlewares.reverse.reduce([response, rest_request, return_value]) do |res_data, middleware|
    ...
    def follow_redirect
      fail Chef::Exceptions::RedirectLimitExceeded if @redirects_followed >= redirect_limit
