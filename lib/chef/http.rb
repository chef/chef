  ...
  class HTTP
    ...
    def retrying_http_errors(url)
      ...
      begin
        loop do
          ...
          if response.is_a?(Net::HTTPServerError) && !Chef::Config.local_mode
            if http_retry_count - http_attempts >= 0
              ...
              Chef::Log.warn("Server returned error #{response.code} for #{url}, retrying #{http_attempts}/#{http_retry_count} in #{sleep_time}s") #Updates from error to warn
              sleep(sleep_time)
              redo
            end
          end
        ...
        if http_retry_count - http_attempts >= 0
          Chef::Log.warn("Error connecting to #{url}, retry #{http_attempts}/#{http_retry_count}") #Updates from error to warn
        ...
        if http_retry_count - http_attempts >= 0
          Chef::Log.warn("Connection refused connecting to #{url}, retry #{http_attempts}/#{http_retry_count}") #Updates from error to warn
          sleep(http_retry_delay)
          retry
        end
        ...
        if http_retry_count - http_attempts >= 0
          Chef::Log.warn("Timeout connecting to #{url}, retry #{http_attempts}/#{http_retry_count}") #Updates from error to warn
          sleep(http_retry_delay)
          retry
        end
        ...
        if (http_retry_count - http_attempts >= 0) && !e.message.include?("certificate verify failed")
          Chef::Log.warn("SSL Error connecting to #{url}, retry #{http_attempts}/#{http_retry_count}") #Updates from error to warn
          sleep(http_retry_delay)
          retry
        end
