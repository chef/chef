      ...
      def get_dependencies(entry)
        if entry.parent && entry.parent.path == '/cookbooks'
          ...
          if role['env_run_lists']
            role['env_run_lists'].each_pair do |_env, run_list|
          ...
          fail Chef::ChefFS::FileSystem::NotFoundError.new(entry)
         else
           []
        end
        ...
        chef_run_list.map do |run_list_item|
          ...
          when :recipe
            if run_list_item.name =~ /(.+)::[^:]*/
              "/cookbooks/#{Regexp.last_match[1]}"
            else
              "/cookbooks/#{run_list_item.name}"
            end
          else
            fail "Unknown run list item type #{run_list_item.type}"
          end
        end
      end
    end
