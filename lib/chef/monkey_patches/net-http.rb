...
module Net
    class HTTP < Protocol
      def connect
        ...
        logger "opening connection to #{conn_addr}:#{conn_port}..."
        ...
        if use_ssl?
          ...
          logger "starting SSL for #{conn_addr}:#{conn_port}..."
          ...
          logger "SSL established, protocol: #{s.ssl_version}, cipher: #{s.cipher[0]}"
        end
        ...
        if s
          logger "Conn close because of connect error #{exception}"
          s.close
        end
        raise
      end
