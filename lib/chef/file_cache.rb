  ...
  class FileCache
    class << self
      ...
      def find(glob_pattern)
        keys = Array.new
        Chef::Util::PathHelper.glob(File.join(file_cache_path, glob_pattern)).each do |f|
          if File.file?(f)
            keys << f[/^#{Regexp.escape(Chef::Util::PathHelper.glob(file_cache_path).first) + File::Separator}(.+)/, 1]
          end
        end
        keys
      end
