  ...
  class ResourceCollection
    ...
    def lookup(resource)
      ...
      if resource.is_a?(Chef::Resource)
        ...
        fail ArgumentError, 'Must pass a Chef::Resource or String to lookup'
      end
      res = @resources_by_name[lookup_by]
      unless res
        fail Chef::Exceptions::ResourceNotFound, "Cannot find a resource matching #{lookup_by} (did you define it first?)"
      end
      @resources[res]
    end
    ...
    def find(*args)
      ...
      args.each do |arg|
        ...
        when String
          ...
          fail Chef::Exceptions::InvalidResourceSpecification, msg
        end
      end
    ...
    def find_resource_by_hash(arg)
      ...
      arg.each do |resource_name, name_list|
        names = name_list.is_a?(Array) ? name_list : [name_list]
        names.each do |name|
    ...
    def find_resource_by_string(arg)
      ...
      when MULTIPLE_RESOURCE_MATCH
        resource_type = Regexp.last_match[1]
        arg =~ /^.+\[(.+)\]$/
        resource_list = Regexp.last_match[1]
      ...
      when SINGLE_RESOURCE_MATCH
        ...
        fail ArgumentError, "Bad string format #{arg}, you must have a string like resource_type[name]!"
      end
      results
    end
    ...
    def is_chef_resource(arg)
      unless arg.is_a?(Chef::Resource)
        fail ArgumentError, "Members must be Chef::Resource's"
      end
      true
    end
  end
