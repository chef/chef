...
darwin_platforms=("macos-14-arm64")
...
for platform in ${darwin_platforms[@]}; do
  echo "- label: \"Integration ${platform#*:} :macos:\""
  echo "  retry:"
  echo "    automatic:"
  echo "      limit: 1"
  echo "  agents:"
  echo "    queue: 'default-${platform%%-*}-${platform##*-}-privileged'"
  echo "  plugins:"
  echo "   - chef/anka#v0.7.2:"
  echo "      vm-name: buildkite-core-${platform#*-}"
  echo "      always-pull: true"
  echo "      wait-network: true"
  echo "      inherit-environment-vars: true"
  echo "  commands:"
  echo "    - \". vault-util-init\""
  echo "    - .expeditor/scripts/bk_container_prep.sh"
  echo "    - .expeditor/scripts/prep_and_run_tests.sh Integration"
  echo "  timeout_in_minutes: 120"
done
...
habitat_plans=("x86_64-linux" "aarch64-linux" "windows" "aarch64-darwin")
for plan in ${habitat_plans[@]}; do
  ...
  case "$plan" in
    ...
    "*-linux")
      ...
      ;;
    "*-darwin")
      echo "    queue: 'default-macos-arm64-privileged'"
      echo "  plugins:"
      echo "   - chef/anka#v0.7.2:"
      echo "      vm-name: buildkite-core-14-arm64"
      echo "      always-pull: true"
      echo "      wait-network: true"
      echo "      inherit-environment-vars: true"
      ;;
