...
describe "openssl checks" do
  ...
  %w{version library_version}.each do |method|
    ...
    example "check #{method}", not_supported_on_macos: true, openssl_version_check: true do
      expect(OpenSSL.const_get("OPENSSL_#{method.upcase}")).to match(openssl_version_default), "#{OpenSSL.const_get("OPENSSL_#{method.upcase}")} OpenSSL doesn't match expected #{openssl_version_default}"
    end
  end
end
