...
describe Chef::Knife::Ssh do
  ...
  describe 'gateway' do
    ...
    context 'when the gateway requires a password' do
      before do
        setup_knife(['-G user@ec2.public_hostname', '*:*', 'uptime'])
        Chef::Config[:knife][:ssh_gateway] = nil
        @knife.session.stub(:via) do |_host, _user, options|
          fail Net::SSH::AuthenticationFailed unless options[:password]
        end
      end
  ...
  def setup_knife(params = [])
    ...
    @api.get('/search/node?q=*:*&sort=X_CHEF_id_CHEF_X%20asc&start=0&rows=1000', 200) do
