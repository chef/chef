...
describe 'Chef::Win32::Registry', :windows_only do
  ...
  describe 'data_exists?' do
    it 'throws an exception if the hive does not exist' do
      lambda { @registry.data_exists?('JKLM\\Software\\Branch\\Flower', name: 'Petals', type: :multi_string, data: %w(Pink Delicate)) }.should raise_error(Chef::Exceptions::Win32RegHiveMissing)
    end
    it 'throws an exception if the key does not exist' do
      lambda { @registry.data_exists?('HKCU\\Software\\Branch\\Flower', name: 'Petals', type: :multi_string, data: %w(Pink Delicate)) }.should raise_error(Chef::Exceptions::Win32RegKeyMissing)
    end
    it 'returns true if all the data matches' do
      @registry.data_exists?('HKCU\\Software\\Root\\Branch\\Flower', name: 'Petals', type: :multi_string, data: %w(Pink Delicate)).should == true
    end
    it 'returns false if the name does not exist' do
      @registry.data_exists?('HKCU\\Software\\Root\\Branch\\Flower', name: 'slateP', type: :multi_string, data: %w(Pink Delicate)).should == false
    end
    ...
    it 'returns false if the data does not match' do
      @registry.data_exists?('HKCU\\Software\\Root\\Branch\\Flower', name: 'Petals', type: :multi_string, data: %w(Mauve Delicate)).should == false
    end
  end
  ...
  describe 'data_exists!' do
    ...
    it 'throws an exception if the name does not exist' do
      lambda { @registry.data_exists!('HKCU\\Software\\Root\\Branch\\Flower', name: 'slateP', type: :multi_string, data: %w(Pink Delicate)) }.should raise_error(Chef::Exceptions::Win32RegDataMissing)
    end
    ...
    it 'throws an exception if the data does not match' do
      lambda { @registry.data_exists!('HKCU\\Software\\Root\\Branch\\Flower', name: 'Petals', type: :multi_string, data: %w(Mauve Delicate)) }.should raise_error(Chef::Exceptions::Win32RegDataMissing)
    end
  end
  ...
  describe 'set_value' do
    ...
    it 'casts an integer string given as a dword into an integer' do
      ...
      @registry.data_exists?('HKCU\\Software\\Root\\Branch\\Flower', name: 'ShouldBe32767', type: :dword, data: 32_767).should == true
    end
    ...
    it 'throws an exception when trying to cast an array to an int for a dword' do
      lambda { @registry.set_value('HKCU\\Software\\Root\\Branch\\Flower', name: 'ShouldThrow', type: :dword, data: %w(one two)) }.should raise_error
    end
    ...
    it 'stores the string representation of an array into a string if you pass it an array' do
      @registry.set_value('HKCU\\Software\\Root\\Branch\\Flower', name: 'ShouldBePainful', type: :string, data: %w(one two)).should == true
    ...
    it 'stores the string representation of a number into a string if you pass it an number' do
      @registry.set_value('HKCU\\Software\\Root\\Branch\\Flower', name: 'ShouldBe65535', type: :string, data: 65_535).should == true
    ...
    it 'throws an exception when a multi-string is passed a number' do
      lambda { @registry.set_value('HKCU\\Software\\Root\\Branch\\Flower', name: 'ShouldThrow', type: :multi_string, data: 65_535) }.should raise_error
    end
