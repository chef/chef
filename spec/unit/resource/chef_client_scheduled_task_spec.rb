...
describe Chef::Resource::ChefClientScheduledTask do
  ...
  describe "#client_cmd" do
    let(:etc_chef_dir) { ChefConfig::Config.etc_chef_dir }

    it "creates a valid command if using all default properties" do
      expect(provider.client_cmd).to eql("#{chef_habitat_binary_path} -L #{etc_chef_dir}/log/client.log -c #{etc_chef_dir}/client.rb")
    end

    it "uses daemon_options if set" do
      resource.daemon_options ["--foo 1", "--bar 2"]
      expect(provider.client_cmd).to eql("#{chef_habitat_binary_path} -L #{etc_chef_dir}/log/client.log -c #{etc_chef_dir}/client.rb --foo 1 --bar 2")
    end
    ...
    it "uses custom log files / paths if set" do
      ...
      expect(provider.client_cmd).to eql("#{chef_habitat_binary_path} -L C:/foo/bar/my-client.log -c #{etc_chef_dir}/client.rb")
    end
    ...
    it "uses custom chef-client binary if set" do
      ...
      expect(provider.client_cmd).to eql("C:/foo/bar/chef-client -L #{etc_chef_dir}/log/client.log -c #{etc_chef_dir}/client.rb")
    end

    it "sets the license acceptance flag if set" do
      resource.accept_chef_license true
      expect(provider.client_cmd).to eql("#{chef_habitat_binary_path} -L #{etc_chef_dir}/log/client.log -c #{etc_chef_dir}/client.rb --chef-license accept")
    end
  end
end
