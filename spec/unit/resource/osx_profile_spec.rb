...
describe Chef::Resource::OsxProfile do
  ...
  describe "action_create" do
    ...
    it "should install when not installed" do
      ...
      provider.load_current_resource
      # Mock profile_installed? to return false since we're testing that it installs when not installed
      allow(provider).to receive(:profile_installed?).and_return(false)
      # Mock write_profile_to_disk to avoid plist conversion issues
      allow(provider).to receive(:write_profile_to_disk).and_return("/tmp/test.mobileconfig")
    ...
    it "does not install if the profile is already installed" do
      ...
      allow(provider).to receive(:get_installed_profiles).and_return(all_profiles)
      provider.load_current_resource
      # Mock profile_installed? to return true since the profile is already installed
