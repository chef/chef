...
describe Chef::Provider::Locale do
    ...
    context "when all locales are available on system" do
      context "with both properties" do
        it "returns an empty array" do
          ...
          expect(provider.send(:unavailable_locales)).to eq([])
        end
      end
    ...
    context "when some locales are not available" do
      context "with both properties" do
        it "returns list" do
          ...
          expect(provider.send(:unavailable_locales)).to eq(["de_DE", "en_US.utf8"])
        end
      end
      context "without lang" do
        it "returns list" do
          resource.lang()
          resource.lc_env({ "LC_TIME" => "en_AG.utf8", "LC_MESSAGES" => "en_US.utf8" })
          expect(provider.send(:unavailable_locales)).to eq(["en_US.utf8"])
        end
      end
      context "without lc_env" do
        it "returns list" do
          resource.lang("de_DE")
          resource.lc_env()
          expect(provider.send(:unavailable_locales)).to eq(["de_DE"])
        end
      end
      context "without both" do
        it "returns an empty array" do
  ...
  describe "#new_content" do
    context "with both properties" do
      ...
      it "returns string" do
        expect(provider.send(:new_content)).to be_a(String)
        expect(provider.send(:new_content)).not_to be_empty
      end
      it "keys will be sorted" do
        expect(provider.send(:new_content).split("\n").map { |x| x.split("=") }.collect(&:first)).to eq(%w{LANG LC_MESSAGES LC_TIME})
      end
      it "ends with a new-line character" do
        expect(provider.send(:new_content)[-1]).to eq("\n")
      end
      it "returns a valid string" do
        expect(provider.send(:new_content)).to eq("LANG=en_US\nLC_MESSAGES=en_AG.utf8\nLC_TIME=en_AG.utf8\n")
      end
    end
    context "without lang" do
      it "returns a valid string" do
        resource.lang()
        resource.lc_env({ "LC_TIME" => "en_AG.utf8", "LC_MESSAGES" => "en_AG.utf8" })
        expect(provider.send(:new_content)).to eq("LC_MESSAGES=en_AG.utf8\nLC_TIME=en_AG.utf8\n")
      end
    end
    context "without lc_env" do
      it "returns a valid string" do
    ...
    context "without both" do
      it "returns string with only new-line character" do
        resource.lang()
        resource.lc_env()
        expect(provider.send(:new_content)).to eq("\n")
      end
    end
  end
  ...
  describe "#up_to_date?" do
    context "when file does not exists" do
      it "returns false" do
        ...
        expect(provider.send(:up_to_date?)).to be_falsy
      end
    end
    ...
    context "when file exists" do
      ...
      context "and contains new key-vals" do
        it "returns true" do
          ...
          expect(provider.send(:up_to_date?)).to be_truthy
        end
      end
    end
  end
