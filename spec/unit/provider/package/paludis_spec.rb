...
describe Chef::Provider::Package::Paludis, "load_current_resource" do
  before(:each) do
    @node = Chef::Node.new
  ...
  describe "when determining the current state of the package" do
    ...
    it "should create a current resource with the name of new_resource" do
      @provider.should_receive(:run_command_with_systems_locale).with({
        :command => "cave -L warning print-ids -s install -m 'dev-scm/git' -s install -f '%v'"
      }).and_return(["1.0.0"])
    ...
    it "should return a package name match from /var/db/pkg/* if a category isn't specified and a match is found" do
      @provider.should_receive(:run_command_with_systems_locale).with({
        :command => "cave -L warning print-ids -s install -m 'dev-scm/*' -s install -f '%v'"
      }).and_return(["1.0.0"])

      @provider = Chef::Provider::Package::Paludis.new(@new_resource_without_category, @run_context)
      @provider.load_current_resource
      @provider.current_resource.version.should == "1.0.0"
    end
  ...
  describe "once the state of the package is known" do
    ...
    describe Chef::Provider::Package::Paludis, "candidate_version" do
...
EOF
    ...
    describe Chef::Provider::Package::Paludis, "install_package" do
      it "should install a normally versioned package using paludis" do
        @provider.should_receive(:run_command_with_systems_locale).with({
          :command => "cave -L warning resolve -x =dev-scm/git-1.0.0"
        })
        @provider.install_package("dev-scm/git", "1.0.0")
      end

      it "should add options to the emerge command when specified" do
        @provider.should_receive(:run_command_with_systems_locale).with({
          :command => "cave -L warning resolve -x --preserve-world =dev-scm/git-1.0.0"
        })
    ...
    describe Chef::Provider::Package::Paludis, "remove_package" do
      it "should uninstall the package with no version specified" do
        @provider.should_receive(:run_command_with_systems_locale).with({
          :command => "cave -L warning uninstall -x dev-scm/git"
        })
        @provider.remove_package("dev-scm/git", nil)
      end

      it "should uninstall the package with a version specified" do
        @provider.should_receive(:run_command_with_systems_locale).with({
          :command => "cave -L warning uninstall -x =dev-scm/git-1.0.0"
        })
        @provider.remove_package("dev-scm/git", "1.0.0")
      end
    end
  end
end
