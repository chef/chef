...
describe Chef::Provider::Package::Yum do
  ...
  let(:new_resource) { Chef::Resource::YumPackage.new("tacos") }

  # Stand in for the yum python helper, which is not available off-platform.
  let(:version) do
    double("version",
      to_s: "tacos-1.0-1.el9.x86_64",
      name: "tacos",
      name_with_arch: "tacos.x86_64",
      version_with_arch: "1.0-1.el9.x86_64")
  end

  let(:python_helper) do
    double("python_helper",
      package_query: version,
      install_only_packages: false,
      compare_versions: 0)
  end

  let(:provider) do
    provider = Chef::Provider::Package::Yum.new(new_resource, run_context)
    allow(provider).to receive(:yum)
    ...
    allow(provider).to receive(:python_helper).and_return(python_helper)
    provider
  end
