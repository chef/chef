...
describe Chef::Client do
  ...
  describe "eol release warning" do
    it "warns when running an EOL release" do
      ...
      client
      expect(Time).to receive(:now).and_return(Time.new(2024, 11, 3, 5))
      allow(client).to receive(:eol_override).and_return(false)
      expect(logger).to receive(:warn).with("This release of Chef Infra Client became end of life (EOL) on Nov 01, 2024. Please update to a supported release to receive new features, bug fixes, and security updates.")
      client.warn_if_eol
    end
