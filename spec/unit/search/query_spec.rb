...
describe Chef::Search::Query do
  ...
  shared_examples "search" do |search_method|
    it "should accept a type as the first argument" do
      lambda { @query.send(search_method,"foo") }.should_not raise_error(ArgumentError)
      lambda { @query.send(search_method, :foo) }.should_not raise_error(ArgumentError)
      lambda { @query.send(search_method, Hash.new) }.should raise_error(ArgumentError)
    end
    ...
    it "should allow a custom query" do
      ...
      @query = Chef::Search::Query.new
      @query.send(search_method,:foo, "gorilla:dundee")
    end
    ...
    it "should let you set a sort order" do
      ...
      @query = Chef::Search::Query.new
      @query.send(search_method,:foo, "gorilla:dundee", "id desc")
    end
    ...
    it "should let you set a starting object" do
      ...
      @query = Chef::Search::Query.new
      @query.send(search_method,:foo, "gorilla:dundee", "id desc", 2)
    end
    ...
    it "should let you set how many rows to return" do
      ...
      @query = Chef::Search::Query.new
      @query.send(search_method, :foo, "gorilla:dundee", "id desc", 2, 40)
    end

    it "should return the raw rows, start, and total if no block is passed" do
      rows, start, total = @query.send(search_method,:foo)
    ...
    it "should call a block for each object in the response" do
      ...
      @query.send(search_method, :foo) { |r| @call_me.do(r) }
    end

    it "should page through the responses" do
      @call_me = mock("blocky")
      @response["rows"].each { |r| @call_me.should_receive(:do).with(r) }
      @query.send(search_method,:foo, "*:*", nil, 0, 1) { |r| @call_me.do(r) }
    end
  end

  describe "search" do
    it_should_behave_like "search" , "search"
  end

  describe "partial_search" do

    it_should_behave_like "search" , "partial_search"
