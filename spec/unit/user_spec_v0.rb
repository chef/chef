...
describe Chef::UserV0 do
  before(:each) do
    @user = Chef::UserV0.new
  end

  describe "initialize" do
    it "should be a Chef::UserV0" do
      expect(@user).to be_a_kind_of(Chef::UserV0)
    end
  end
  ...
  describe "when deserializing from JSON" do
    before(:each) do
      user = { "name" => "mr_spinks",
        ...
        "password" => "password",
        "admin" => true }
      @user = Chef::UserV0.from_json(Chef::JSONCompat.to_json(user))
    end

    it "should deserialize to a Chef::UserV0 object" do
      expect(@user).to be_a_kind_of(Chef::UserV0)
    end
  ...
  describe "API Interactions" do
    ...
    describe "list" do
      before(:each) do
        ...
        allow(Chef::UserV0).to receive(:load).with("admin").and_return(@user)
      ...
      it "lists all clients on an OSC server" do
        ...
        expect(Chef::UserV0.list).to eq(@osc_response)
      end

      it "inflate all clients on an OSC server" do
        allow(@http_client).to receive(:get_rest).with("users").and_return(@osc_response)
        expect(Chef::UserV0.list(true)).to eq(@osc_inflated_response)
      end

      it "lists all clients on an OHC/OPC server" do
        allow(@http_client).to receive(:get_rest).with("users").and_return(@ohc_response)
        # We expect that Chef::UserV0.list will give a consistent response
