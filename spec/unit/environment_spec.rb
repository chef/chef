...
describe Chef::Environment do
  ...
  describe 'self.validate_cookbook_versions' do
    ...
    it 'should validate the version string of each cookbook' do
      @cookbook_versions.each do |_cookbook, version|
  ...
  describe 'self.validate_cookbook_version' do
    ...
    describe 'in solo mode' do
      ...
      it 'should raise and exception' do
        ...
        end.should raise_error Chef::Exceptions::IllegalVersionConstraint,
  ...
  describe 'when updating from a parameter hash' do
    ...
    it 'validates the name given in the params' do
      ...
      @environment.invalid_fields[:name].should == "Option name's value @$%^&*() does not match regular expression /^[\-[:alnum:]_]+$/"
    end
    ...
    it 'updates default attributes from a JSON string in params[:attributes]' do
      @environment.update_from_params(:name => 'fuuu', :default_attributes => '{"fuuu":"RAGE"}')
      @environment.default_attributes.should == { 'fuuu' => 'RAGE' }
    end

    it 'updates override attributes from a JSON string in params[:attributes]' do
      @environment.update_from_params(:name => 'fuuu', :override_attributes => '{"foo":"override"}')
  ...
  describe 'when loading' do
    describe 'in solo mode' do
      ...
      it 'should raise an error if the configured environment_path is invalid' do
        ...
        lambda do
          Chef::Environment.load('foo')
        end.should raise_error Chef::Exceptions::InvalidEnvironmentPath, "Environment path '/var/chef/environments' is invalid"
      end
      ...
      it 'should raise an error if the file does not exist' do
        ...
        lambda do
          Chef::Environment.load('foo')
        end.should raise_error Chef::Exceptions::EnvironmentNotFound, "Environment 'foo' could not be loaded from disk"
      end
    end
  end
