...
describe Chef::Provider::Package::Pkgin, "load_current_resource" do
  before(:each) do
    @node = Chef::Node.new
    ...
    @current_resource = Chef::Resource::Package.new("varnish")
	  ...
	  @status = mock("Status", :exitstatus => 0)
		@provider = Chef::Provider::Package::Pkgin.new(@new_resource, @run_context)
    ...
    it "run pkgin and install the package" do
      ...
      @provider.should_receive(:shell_out!).with("pkgin avail | grep ^varnish-[0-9] | awk '{ print $1 }'", :env => nil, :returns => [0,1]).and_return(@shell_out)
